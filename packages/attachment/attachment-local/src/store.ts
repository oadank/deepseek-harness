...
export async function commitPreparedImageFile(
  ...
  try {
    ...
    // [2026-08-22] 适配上游 prepareImageFile→commitPreparedImageFile 重构（prepared.ref.mediaType / prepared.data）
    const alias = extensionAliasPath(target, prepared.ref.mediaType)
    if (alias !== null && !(await existsSync(alias))) {
      try {
        if (prepared.ref.mediaType === 'image/webp') {
          const { default: sharpMod } = await import('sharp')
          await sharpMod(prepared.data).png().toFile(alias)
        } else {
