...
> | 45 | `packages/host/edge-tts/`（**新增 Host 包**）<br>`packages/bundle/web-app/cordis.patch.yml` + `package.json` | **Edge TTS 迁到 0.1.5 Connection Fetch 路由**：`POST /api/edge-tts`（JSON `{text,voice?}` → `audio/mpeg`）。收流超时 **12s** + 自动重试 **最多 3 次**（500ms/1s 递增）参数原样保留。官方无等价内置 TTS | 旧 apiproxy 被 0.1.5 删除；TTS 必须挂在 `ctx.connection.fetch.register`（与 session.export 同模式） | ✅ **0.1.5 已迁移**（待构建/重启验证） |
