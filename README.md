...
一个给 **DeepSeek Harness**（本地 AI 助手）装上"嘴巴和耳朵"的版本：

- 🎙️ **能听**：你说话 → 自动识别成文字
- 🔊 **能说**：AI 用语音回复你（微软免费 / 小米 / 本地离线，三选一）
- 🗣️ **克隆音色**：用你自己的声音说话
- 👀 **能看图**：模型看不了图时，自动交给本地视觉模型识别
...
> ⚠️ **重要：别被 `npx` 误导——它不是安装**
>
> 官方文档写的 `npx @deepseek-ai/dsh web` 是**临时运行**，不是正式安装：
> - npx 会把包下载到 npm 缓存（`~/.npm/_npx`）临时拉起来跑，**不会装出全局 `dsh` 命令**，每次都要 npx 重新解析；
> - 想**正式安装**官方版，请用：`npm install -g @deepseek-ai/dsh`，之后直接运行 `dsh web`；
> - 不管临时还是正式，你的数据（会话、配置）都存在 `~/.dsh`，**不会丢**；
> - 本项目按下面「一、安装」的步骤走，装完就是正式安装，不存在这个问题。
