...
> | 51 | `client/connection/src/browser-auth.ts`<br>`connection/tsdown.config.ts` `hostPhase: true` | **固定 Web 登录 token**：设环境变量 `DSH_WEB_TOKEN`（≥16 字符）后，登录 URL 跨重启不变；未设则仍随机。**不破坏 trust 栅栏**（trustedHosts/Host/Origin/cookie 签名不变）。配置见下「固定 token 用法」 | 每次重启 token 变导致 tailscale 收藏链接失效 | ✅ **0.1.5 已迁移**（待重启验） |
> > **固定 token 用法（任务 B）**：在 nssm 服务环境加 `DSH_WEB_TOKEN=<你的固定串≥16字符>`（`nssm set dsh-web AppEnvironmentExtra ...`，与现有 PATH 整值合并写），重启 dsh-web 后打印的 URL 即固定。未配置则保持随机 process token。勿把 token 提交到公开仓库。
