...
> | 55 | 插件 `dsh-input-tools/lib/client.js`（无独立 git，记本表） | **工具条顺序**：① CSS 由 `:has([data-composer-left])` 改为 `*[data-composer-left]`（attr 在元素自身，:has 只匹配子孙故 0.1.5 不生效）；② `.modes`（完全权限）order=10 介于 left(0) 与其它(20) 之间 → 图片/录音在权限左侧。**官方「添加附件」📎 保留不隐藏**（此前「隐藏📎」任务作废）。浏览器 **Ctrl+F5** 刷新插件 client 即可 | 0.1.5 工具条顺序仍是 权限在图片前；:has 选择器打不中自身 attr | ✅ 插件已改，待浏览器验证 |
